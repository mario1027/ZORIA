...
def update_sweep(n_intervals, sweep_n, stop_n, cancel_n, start, end, points, scale, 
    ...
    global sweep_thread, sweep_data_global, stop_sweep
    ...
    if param:
        ...
        bode_fig = make_subplots(
            rows=2, cols=1,
            subplot_titles=('Magnitud de Impedancia', 'Fase de Impedancia'),
            vertical_spacing=0.12,
            row_heights=[0.5, 0.5],  # Ambos subplots con mismo tamaño
            specs=[[{"type": "scatter"}], [{"type": "scatter"}]]
        )
        bode_fig.add_trace(
            go.Scatter(x=param, y=z_mag, mode='lines+markers', name='|Z|',
                      line=dict(color='#1f77b4', width=2),
                      marker=dict(size=6)),
            row=1, col=1
        )
        bode_fig.add_trace(
            go.Scatter(x=param, y=phase, mode='lines+markers', name='θ',
                      line=dict(color='#ff7f0e', width=2),
                      marker=dict(size=6)),
            row=2, col=1
        )
        bode_fig.update_xaxes(title_text="Frecuencia (Hz)", type="log", row=2, col=1)
        bode_fig.update_yaxes(title_text="|Z| (Ω)", type="log", row=1, col=1)
        bode_fig.update_yaxes(title_text="Fase (°)", row=2, col=1)
    ...
    if z_real and z_imag:
        ...
        nyquist_fig.update_yaxes(scaleanchor="x", scaleratio=1)
